...
const Analytics = () => {
  ...
  const loadComputerPrediction = async (compId) => {
    ...
    try {
      ...
      if (Array.isArray(histList) && histList.length > 0) {
        histList.slice().reverse().forEach((m, idx) => {
          ...
          combined.push({
            ...
            predictedScore: null, // No prediction for historical points
            isPrediction: false
          });
        });
      } else {
        // Fallback smooth historical curve if history empty
        for (let i = 30; i >= 1; i--) {
      ...
      combined[combined.length - 1].predictedScore = lastActual; // Bridge point for chart connection

      // Future predicted trend (Dashed line)
      const baseTrend = (predData?.riskScorePercentage || 82) > 60 ? 1.4 : 0.2;
      for (let i = 1; i <= 6; i++) {
        const futureDate = new Date(now.getTime() + i * 10 * 24 * 60 * 60 * 1000);
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="space-y-2 text-center md:text-left">
              <div className="flex items-center gap-2">
                ...
                <span className={`px-2.5 py-0.5 rounded-full text-xs font-extrabold ${(predictionData?.riskScorePercentage || 82) > 60 ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'}`}>
                  {predictionData?.riskLevel || 'HIGH'} RISK
                </span>
              </div>
              <h3 className="text-3xl font-black text-slate-100">
                {predictionData?.predictedIssue || 'Performance Degradation Risk'}
              </h3>
              <p className="text-xs text-slate-400 max-w-xl">
                Estimated timeframe: <strong className="text-slate-200">{predictionData?.estimatedTimeframe || '~2 months'}</strong> • Confidence: <strong className="text-cyan-400">{predictionData?.riskScorePercentage || 82}%</strong>
              </p>
            </div>

            {/* Risk Gauge Badge */}
            <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 text-center space-y-1 min-w-[140px]">
              <span className="text-3xl font-black text-cyan-400">{predictionData?.riskScorePercentage || 82}%</span>
          ...
          <div className="p-4 rounded-2xl bg-slate-950/60 border border-slate-800/80 space-y-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs">
              <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-cyan-400" /> Increasing memory allocation trend
              </div>
              <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-blue-400" /> High disk storage capacity utilization
              </div>
              <div className="p-2.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 flex items-center gap-2">
                <span className="w-2 h-2 rounded-full bg-purple-400" /> Active background process count escalation
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="h-80 w-full pt-4">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={trendChartData}>
              ...
              {/* Predicted Future Trend Line (Dashed) */}
              <Line
                type="monotone"
